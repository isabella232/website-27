...
Event [SpeedChanged](), [AttitudeChanged] and [PositionChanged]() (only if gps of the drone has fixed) are triggered.<br/>
...
Then, event [ReturnHomeState](#1-4-3]) is triggered.<br/>
...
Then, event [RelativeMoveEnded](#1-34-0]) is triggered.<br/>
If you send a second relative move command, the drone will trigger a [RelativeMoveEnded](#1-34-0]) with the offsets it managed to do before this new command and the value of error set to interrupted.<br/>
...
### <a name="ARDrone3-Camera-Orientation">Move the camera</a><br/>
> Move the camera:
...
Then, event [VideoState](#1-8-3]) will be triggered. Also, notification [VideoEvent](#ARDrone3-MediaRecordEvent-VideoEventChanged) is triggered.<br/>
