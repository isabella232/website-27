...
Here are, for each products, the different variants and task we support:
...
The command to build the SDK for all iOS variants is:<br/>
...
The command to build the SDK for all Android variants is:<br/>
...
+-SDK/
  ...
  |
  +-build.sh                    Top level build script that can invoke everything (symlink)
  |
  ...
  +-packages/
  |    |
  |    +-ARSDKBuildUtils/       All the tools to create the autotools needed to build the SDK
  |    |
  |    +-ARSDKTools/            External libs used by the SDK
  |    |
  |    +-Docs/                  SDK doxygen
  |    |
  |    +-libARCommands/         All the sources related to the commands you can send to and events you can receive from the drone
  |    |
  ...
  |    |
  |    +-libARMedia/            Abstraction layer around the medias generated by the drones
  |    |
  |    +-libARNetwork/          Send and receive packets to/from the drone
  |    |
  |    +-libARNetworkAL/        Abstraction layer around the different networks (BLE or Wifi networks)
  |    | 
